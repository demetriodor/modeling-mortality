...
As you can see, getting the mortality data per week for the Netherlands is quite straightforward. But if you don't want to bother with getting the data yourself, you can access the complete dataset used in the analysis [here](http://github.com). 
...
nlw.w <- read.zoo (nlw)
nlw.w <- apply.weekly(nlw.w, mean, na.rm=T)
nlw.w$week<-as.numeric(as.character(strftime(nlw.w,format = "%V")))
nlw.w$year<-as.numeric(as.character(strftime(nlw.w,format = "%Y")))
...
nlw.w  = data.frame(nlw.w) %>% 
  mutate (temp.max = round(as.numeric(as.character(TMAX)), 2),
          ...
          id = paste0(year, '.', week),
          id = case_when(id == '2010.53' ~ '2010.52', id == '2016.53' ~ '2016.52', TRUE ~ id) # recode weeks 53 to 52
          ) %>% 
  dplyr::select (id, temp.max, temp.min) 

# create variables to use in the models
nlw.w$t.min<-ifelse(nlw.w$temp.min>0, 0, 0-nlw.w$temp.min)
nlw.w$t.max<-ifelse(nlw.w$temp.max<20, 0, nlw.w$temp.max-20)

nlw.w<-nlw.w[-c(209,470),] # two observations need to be removed due to the way weeks are constructed
```
